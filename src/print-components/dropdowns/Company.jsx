...
const Company = ({ companyData, companies, setCompanies, isMt }) => {
  ...
  const firstCompanyName = companyData
    ?.filter((item) => item.companyid === companies[0])
    .map((item) => item.companyname)
    .join(" ");

  useEffect(() => {
    setFilteredSuggestions(companyData);
  }, [companyData]);
  ...
  const handleInputChange = (e) => {
    ...
    const filtered = companyData.filter((suggestion) =>
  ...
  const handleClickAll = () => {
    const allCompanyIds = companyData?.map(
      (suggestion) => suggestion.companyid
    );
                ...
                {companies?.length > 0 &&
                  companies.map((selectedCompanyId) => {
                    const selectedSuggestion =
                      ...
                      companyData.find(
                        (suggestion) =>
                          suggestion.companyid === selectedCompanyId
                      );
